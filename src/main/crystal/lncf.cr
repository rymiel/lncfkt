...
module LNCF
  br = Bytecode.new(IO::Hexdump.new(File.open("test.lncfb"), output: STDERR, read: true)).read
  c = br.const_pool
  d = br.defined
  h = br.header
  ...
  vm = VM.new c, {
    "lib" => {
      "map_characters" => Lib.auto_lib(map_characters),
      "has_match" => Lib.auto_lib(has_match),
      "match_count" => Lib.auto_lib(match_count),
      "replace" => Lib.auto_lib(replace)
    },
    "op" => {
      "append" => Lib.auto_lib(op_append),
      "char" => Lib.auto_lib(op_char)
    },
    "local" => d.transform_values { |i| VM.defined_method(*i) }
  }
  ...
  if ARGV[0]? == "run"
    ...
    word = ARGV[1]
    puts ">> #{word}"
    manual_enums = Hash(Bytecode::EnumDefinition, {String, Int32}).new
    ...
    classified = Hash(Bytecode::Classification, String).new
